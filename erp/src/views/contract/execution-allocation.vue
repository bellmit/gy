...
<template>
  <div class="paydetail-wrapper">
    <div class="essential-information">
      ...
      <cont :list="list" ref="contEssVo" @chkPerson="chkPerson" @chkCompany="chkCompany" :isZXFP="isZXFP"></cont>
    </div>
          ...
          <template v-if="isDistribution === 1">
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">上游计划号</div>
                <div class="essential-text">
                  <div class="essential-text">
                    <input type="text" @blur="essenceTest(eaSubmit.purchasePlanNumber, 'purchasePlanNumber')" v-model="eaSubmit.purchasePlanNumber" placeholder="请输入上游计划号"/>
                  </div>
                </div>
              </div>
            </div>
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">上游统计口径</div>
                <div class="essential-text">
                  <div class="essential-text">
                    <el-select v-model="eaSubmit.purchaseStatisticalCaliber" placeholder="请选择">
          ...
          <template v-else>
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">上游计划号</div>
                <div class="essential-text">
                  <div class="essential-text">{{list.sellerInfo.planNumber}}</div>
                </div>
              </div>
            </div>
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">上游统计口径</div>
                <div class="essential-text">
                  <div class="essential-text">{{list.sellerInfo.statisticalCaliber}}</div>
                </div>
              </div>
            </div>
          </template>
      ...
      <div class="receivables-information" v-if="list.contractType === 1 || list.contractType === 3">
        ...
        <div class="essential-wrapper">
          ...
          <template v-if="isDistribution === 1">
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">下游计划号</div>
                <div class="essential-text">
                  <div class="essential-text">
                    <input type="text" @blur="essenceTest(eaSubmit.salePlanNumber, 'salePlanNumber')" v-model="eaSubmit.salePlanNumber" placeholder="请输入下游计划号"/>
                  </div>
                </div>
              </div>
            </div>
            <div class="essential-row">
              <div class="essential-item">
                ...
                <div class="essential-text">
                  <div class="essential-text">
                    <el-select v-model="eaSubmit.saleStatisticalCaliber" placeholder="请选择">
                      <el-option v-for="item in statisticalCaliber" :key="item"
                                 :label="item" :value="item">
                        <span>{{item}}</span>
                      </el-option>
          ...
          <template v-else>
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">下游计划号</div>
                <div class="essential-text">
                  <div class="essential-text">{{list.buyerInfo.planNumber}}</div>
                </div>
              </div>
            </div>
            <div class="essential-row">
              <div class="essential-item">
                <div class="essential-title">下游统计口径</div>
                <div class="essential-text">
                  <div class="essential-text">{{list.buyerInfo.statisticalCaliber}}</div>
                </div>
              </div>
            </div>
          </template>
...
export default {
    ...
    watch: {
        eaSubmit: {
            handler (val) {
                if (val.salePlanNumber && val.salePlanNumber.length > 20) {
                    this.$message({
                        message: '下游计划号最多可填写20字',
                        type: 'error'
                    });
                    this.eaSubmit.salePlanNumber = this.eaSubmit.salePlanNumber.substr(0, 20);
                    return;
                }
                if (val.purchasePlanNumber && val.purchasePlanNumber.length > 20) {
                    this.$message({
                        message: '上游计划号最多可填写20字',
                        type: 'error'
                    });
                    this.eaSubmit.purchasePlanNumber = this.eaSubmit.purchasePlanNumber.substr(0, 20);
                }
            },
            deep: true
        },
    ...
    methods: {
        getdetailOrder () { // 合同要素详情
            this.$http.get(this.$api.payment.getDetailOrder + this.$route.query.id).then((response) => {
        ...
        Sub () {
            if (this.eaSubmit.executiveId === null) {
                this.$message.error('请先选择执行人员');
                return;
            }
            if (this.eaSubmit.targetCorpId === null) {
                this.$message.error('请先选择我方公司');
                return;
            }
        ...
        essenceTest (e, type) {
            this.$http.post(this.$api.contract.essenceTest, {planNumber: e})
                .then((res) => {
                    if (res.data.code === 1) {
                        this.$message.error('计划号已存在，请重新输入');
                        this.eaSubmit[type] = null;
                    }
                })
                .catch((e) => {
                    console.log(e);
