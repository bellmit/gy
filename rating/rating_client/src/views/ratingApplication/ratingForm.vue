...
<template>
  <div class="application">
       <!-- 基础信息 -->
       <div class="gy-form ">
            <p class="model_title"><i class="el-icon-tickets"></i> 公司信息</p>
            <div class="gy-form-group">
                <span class="l isRequired">公司名称</span>
                <span  v-if="applyType === 0">{{ratingFormData.name}}</span>
                <span v-else>
                    <el-select
            ...
            <div  v-if="applyType !== 0"  class="company_type divPadding gy-form-group">
                <span class="l">申报类型</span>
                 <el-select  class="single_select " @change="ratingFormData.declareChildIdList = []" v-model="ratingFormData.declareParentId"  placeholder="请填写">
                    <el-option
                 ...
                 <el-select  class="multiple_select" v-model="ratingFormData.declareChildIdList"   collapse-tags multiple placeholder="请填写">
                    <el-option
     ...
     <!-- 联系人信息 -->
    <div class="gy-form">
           <p class="model_title"><i class="el-icon-tickets"></i> 联系人信息</p>
             <div class="contact_info clearfix" v-for="(item, index) in ratingFormData.companyContactModelList" :key="index">
                ...
                <div class="gy-form-group">
                    <span class="l"></span>
                    <input  type="text" placeholder="详细地址" class="gy-input" v-model="item.address">
                    <!-- <textarea class="gy-textarea text-address" placeholder="详细地址" ref="textareaAddRow" @keyup.enter="setHeight1"  v-model="item.address" ></textarea> -->
                </div>
                <div class="gy-form-group ">
     ...
     <!-- 银行信息 -->
    <div class="gy-form">
           <p class="model_title"><i class="el-icon-tickets"></i> 银行信息</p>
     ...
     <div class="gy-form">
            ...
            <div  class="company_type gy-form-group">
                <span class="l">企业类型</span>
                 <el-select  @change="ratingFormData.enterpriseChildId=''"  v-model="ratingFormData.enterpriseParentId"  placeholder="请填写">
                    <el-option
           ...
           <div class="add_list_data gy-button-normal" @click="addTableData">新增</div>
            <table v-if="ratingFormData.enterpriseParentId==10" class="gy-table product_list">
                ...
                <tr v-for="(item,index) in productList" :key="index">
                    ...
                    <td><input  maxlength='7' placeholder="请输入数字" @change="toNum('productList',item.productionOutput,'productionOutput', index)" v-model="item.productionOutput"></td>
        ...
        <div class="gy-form  clearfix">
              <p class="model_title"><i class="el-icon-tickets"></i> 证件信息</p>
      ...
      <div class="gy-form submit_con">
          <div class="submit_div">
            <div @click="sendFormData(0)" class="gy-button-normal">保存</div>
...
export default {
    ...
    methods: {
        init () {
            ...
            if (this.applyType === 0) {
                this.ratingFormData.name = JSON.parse(localStorage.getItem('userInfoClient')).companyName;
                this.ratingFormData.companyId = JSON.parse(localStorage.getItem('userInfoClient')).companyId;
            }
        },
        addContact () {
            let contactInfo = {
        ...
        getRatingType () { // 获取公司类型列表
            ...
            this.$http.get(this.$api.ratingApplication.declareInfo).then((res) => {
                if (res.data.code === 0) {
                    let data = res.data.data;
                    that.baseInfoCompanyType.declareParentId = data;
                } else {
        ...
        getPriseType () { // 获取企业类型
            ...
            this.$http.get(this.$api.ratingApplication.enterpriseTypeList).then((res) => {
                if (res.data.code === 0) {
                    let data = res.data.data;
                    that.enterPriseType.enterpriseParent = data;
                } else {
                    that.$message(res.data.message);
                }
        ...
        getCompanyInfo () {
            this.$http.get(this.$api.ratingApplication.getCompanyInfo).then((res) => {
                if (res.data.code === 0) {
                    ...
                    if (data.companyContactModelList && data.companyContactModelList.length > 0) {
                        this.ratingFormData.companyContactModelList = data.companyContactModelList;
                    }
                    this.setContactAreasList();
                    console.log(this.getFormData.address);
        ...
        setHeight1 () {
            // if (!this.settedLine1) this.settedLine1 = true;
            console.log(this.$refs.textareaAddRow);
            this.$refs.textareaAddRow.height = '60px';
        },
        getFormData () {
            this.$http.get(this.$api.ratingApplication.getClientRatingFormData + this.currentCompanyId).then((res) => {
                if (res.data.code === 0) {
                    ...
                    this.setTextAreaheight(this.ratingFormData.address);
                    // console.log(JSON.stringify(this.ratingFormData));
                }
            });
        },
        setHeight () {
