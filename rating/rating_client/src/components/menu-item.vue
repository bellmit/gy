<template>
  <div>
    <template v-for="item in menuData">
      ...
      <el-menu-item @click.native="goPage({name: item.frontUrl})"  :index="item.name" :key="item.id">
          <i v-if="item.iconClass" :class="[item.iconClass]"></i>&nbsp;<span slot="title">{{ item.name }}</span>
      </el-menu-item>
    </template>
  </div>
</template>
