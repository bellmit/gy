<template>
    <div class="saleschaseBills">
        ...
        <div class="content">
            ...
            <div v-if="!supply && showBillIcon != 3">
            ...
            <div v-if="supply || showBillIcon == 3" style="margin-top:20px;"></div>
            ...
            <!-- <div class="gy-form-group"> -->
                <!-- <div class="uploadbox" v-for="(item, index) in InvoiceUrlList" :key="index">
                    <img src="../../assets/images/pdf.png" alt="" @click="magnifyImg($event)">
                </div> -->
                <!-- 上传发票 -->
                <!-- <div>
                    <div>
                        <el-upload
                            multiple
                            :disabled="showBillIcon == 3"
                            :class="[{'hide-btn': showBillIcon == 3 || limit === InvoiceUrlList.length}, 'demo']"
                            list-type="picture-card"
                            :data="{'storage':'logistics'}"
                            :action="imgApi"
                            :limit="limit"
                            :on-success="addthImagesuccessfp"
                            :file-list="InvoiceUrlList"
                            :on-preview="handlePictureCardPreviewfp"
                            :on-remove="handleRemovefp">
                            <i class="el-icon-plus"></i>
                        </el-upload>
                        <el-dialog :visible.sync="dialogVisible">
                            <img width="100%" :src="dialogImageUrl" alt="">
                ...
                <!-- 结束 -->
            <!-- </div> -->
            <div class="c-title cl"><i class="iconfont icon-zijinzhanghuicon" style="font-size:12px"></i><span>收款凭证</span></div>
            <div class="table-box">
                <table class="gy-table">
                    <thead>
                        <th style="width:14%">收款单号</th>
                        <th style="width:13%">收款时间</th>
                        <th style="width:9%">收款金额</th>
                        <th style="width:9%">收款类型</th>
                        <th style="width:9%">收款方式</th>
                        <th style="width:9%">收款状态</th>
                        <th style="width:20%">收款凭证</th>
                    </thead>
                    <tbody>
            ...
            <div class="mytable">
                <table class="gy-table">
                    ...
                    <tr v-for="(items, ind ) in item.deliveryOrderItemList" :key="ind">
                        ...
                        <td>
                            <!-- <div v-if="item.tradeMode !== 1" v-for="(items, index) in item.deliveryOrderItemList" :key="index" >
                                <img width="40px" height="40ox" :src="items.imagePath" alt="">
                            </div> -->
                            <div v-if="item.tradeMode !== 1" class="" @click="canshu(item, '1')">
                                <el-upload
                                    multiple
                                    list-type="picture-card"
                                    :disabled="showBillIcon == 3"
                                    :class="[{'hide-btn': showBillIcon == 3 || limit === propertyList[index].deliveryOrderFileList.length}, 'demo']"
                                    :data="{'storage':'logistics'}"
                                    :action="imgApi"
                                    :limit="limit"
                                    :on-success="addthImagesuccesshq"
                                    :on-preview="handlePictureCardPreviewhq"
                                    :file-list="propertyList[index].deliveryOrderFileList"
                                    :on-remove="handleRemovehq">
                                    <i class="el-icon-plus"></i>
                                </el-upload>
                                <el-dialog :visible.sync="dialogVisible">
                                    <img width="100%" :src="dialogImageUrl" alt="">
...
export default {
    ...
    methods: {
                            ...
                            item.fileUrl.map(function (item) {
                                ...
                                if (item.fileUrl.indexOf('http') === -1) {
                                    ...
                                    if ((item.fileUrl.indexOf('.pdf') !== -1)) {
                                        item.fileUrl = process.env.API_ROOT_MAIN + that.$api.payment.paymentImage + '?filePath=' + item.fileUrl + '&t=' + timestmp;
                                    }
        ...
        // 上传货权凭证
        addthImagesuccesshq (file, fileList) {
            this.property.newurl = file[0].filePath;
            console.log(this.property);
            this.$http.post(this.$api.payment.deliveryBillsupload, this.property)
                .then(res => {
                    console.log(res);
                    this.getPropertyList();
                });
        ...
        handleRemovehq (file, fileList) {
            ...
            this.$http.post(this.$api.payment.deliveryBillsupload, file)
                .then(res => {
                    console.log(res);
                });
        },
        ...
        // 上传发票
        addthImagesuccessfp (file, fileList) {
            console.log(file);
            file[0].newurl = file[0].filePath;
            file[0].orderId = this.orderID.odrOrderId;
            file[0].userId = JSON.parse(localStorage.getItem('userInfo')).id;
            console.log(file);
            this.$http.post(this.$api.payment.invoiceupload, file[0])
                .then(res => {
                    console.log(res);
                    this.getInvoiceUrlList();
                });
        ...
        handleRemovefp (file, fileList) {
            ...
            this.$http.post(this.$api.payment.invoiceupload, file)
                .then(res => {
                    console.log(res);
                });
        }
