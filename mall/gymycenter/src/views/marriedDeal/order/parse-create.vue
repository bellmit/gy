<template>
    <div class="transport-wrap married-wrapper">
        ...
        <div class="detail-box">
            <p class="note"><span>温馨提示： </span>通过下方文本框文字识别，智能生成底部“撮合订单详情”要素；同时支持手动编辑撮合订单详情</p>
            <h3><i class="iconfont icon-info"></i>识别确认信息</h3>
            <div class="gy-form parse-form">
                <div class="textarea">
                    <textarea v-model="content" class="gy-textarea" placeholder="请输入或复制、粘贴撮合成交信息，系统将自动识别并转换为订单信息。 例如：                                                                                                                                                                                                                                                                                                         卖方:  ***  买方:  ***                                                                                                                                                                                                                                                                                                         成交细节: 乙二醇  4900元    4月20-30日    长江国际    进口    免仓7    1000吨    10%保证金    先款后货"></textarea>
                </div>
                <div class="gy-form-button parse-button">
                    <a href="javascript:;" class="gy-button-extra" @click="parseGet">识别</a>
                </div>
            </div>
        </div>
        <div class="detail-box">
            ...
            <div class="gy-form">
                ...
                <div class="gy-form-button">
                    <a href="javascript:;" class="gy-button-extra" @click="formSubmit">提交</a>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getDetail () {
            this.orderId && this.$http.get(this.$api.marriedDeal.create + '/' + this.orderId)
                .then(res => {
                    ...
                    this.disableDay = res.data.data.deliveryDateMonth === 0;
        ...
        handleSelectProduct (item) {
            ...
            this.createData.productId = item.productId;
        ...
        formSubmit (t) {
            ...
            // if (!this.createData.warehouseFreeDays) {
            ...
            this.createData.dmkBuyerPayAmount = !this.createData.dmkBuyerPayAmount ? 0 : this.createData.dmkBuyerPayAmount;
            this.createData.dmkSellerPayAmount = !this.createData.dmkSellerPayAmount ? 0 : this.createData.dmkSellerPayAmount;
            this.createSend();
        },
        createSend () {
        ...
        parseGet () {
            let str = this.content.replace('%', '%25');
            this.$http.get(this.$api.marriedDeal.parse + '?content=' + encodeURI(str))
                .then(res => {
                    if (res.data.code === 0) {
                        console.log(res.data.data.deliveryDateMonthType === null);
                        this.buyerCompanyName = res.data.data.buyerCompanyName;
                        this.sellerCompanyName = res.data.data.sellerCompanyName;
                        this.warehouseName = res.data.data.deliveryWarehouseName;
                        this.productName = res.data.data.skuName;
                        ...
                        this.createData.dmkBuyerPayAmount = 0;
                        this.createData.dmkSellerPayAmount = 0;
                        this.createData.productId = res.data.data.skuInfoId;
                        return;
                    }
                    this.$alert(res.data.message, '出错了', {type: 'error'});
