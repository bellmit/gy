<template>
    <div class="transport-wrap married-wrapper">
        <div class="main-title">
            <h2>撮合订单详情<span>订单号：{{detailInfo.dmkOrderSn}}</span></h2>
            <span class="status">状态：{{statusValue[detailInfo.dmkOrderStatus]}}</span>
        </div>
        <div class="detail-box">
            <h3><i class="iconfont icon-info"></i>撮合订单信息</h3>
            <div class="gy-form">
                ...
                <div class="gy-form-button" v-if="detailInfo.companyType === 2">
                    ...
                    <router-link v-if="detailInfo.dmkOrderStatus === 1" :to="{ name: 'reAdd', query: {orderId: detailInfo.id, typeId: 4} }" class="gy-button-extra">发起订单</router-link>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getDetail () {
            this.$http.get(this.$api.marriedDeal.create + '/' + this.$route.query.id)
                .then(res => {
