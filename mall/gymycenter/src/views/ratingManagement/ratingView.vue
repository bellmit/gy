...
<template>
    <div class="application">
        <div class="new-title-public">评级材料信息</div>
        <!-- 基础信息 -->
        <div class="clearfix module-wrapper basic-info">
            <p class="model_title company-information-view company-information"><i class="el-icon-tickets"></i> 公司信息</p>
            ...
            <div class="gy-form-group">
                <span class="l" style="width: 146px">统一社会信用代码</span>
                <span>{{viewData.socialCode|dataFormat}}</span>
            </div>
            <div class="gy-form-group">
                <span class="l">固定资产(万元)</span>
                <span>{{viewData.fixedAssets}}</span>
            </div>
            <div class="gy-form-group">
                <span class="l">年销售规模(万元)</span>
                <span>{{viewData.salesVolume}}</span>
            </div>
        </div>
        <!-- 联系人列表 -->
        <div style="padding-right:16px;" class="clearfix module-wrapper">
            <p class="model_title rating-contacts"><i class="el-icon-tickets"></i> 联系人信息</p>
            <table class="gy-table product_list">
                ...
                <tbody>
                <tr v-for="(item,index) in viewData.companyContactModelList" :key="index">
                    <td>{{item.name|dataFormat}}</td>
                    <td>{{item.contactAddress|dataFormat}}</td>
                    <td>{{item.phone|dataFormat}}</td>
                    <td>{{item.position|dataFormat}}</td>
                    <td>{{item.identityCardNumber|dataFormat}}</td>
                </tr>
                </tbody>
            </table>
        </div>
        <!-- 银行信息 -->
        <div class="clearfix module-wrapper bank-info">
            <p class="model_title company-information-view"><i class="el-icon-tickets"></i> 银行信息</p>
            <div class="clearfix" v-for="(item, index) in viewData.companyBankInfoModelList" :key="index+800">
                <div class="gy-form-group" style="padding-left:114px;">
                    <span class="l">开户行</span>
                    <span>{{item.openBank|dataFormat}}</span>
                </div>
                <div class="company_type divPadding gy-form-group" style="padding-left:114px;">
        ...
        <!-- 税务信息 -->
        <div class="clearfix module-wrapper identity-info">
            <p class="model_title"><i class="el-icon-tickets"></i> 相关人身份信息</p>
            <div class="clearfix">
                <div class="gy-form-group">
        ...
        <div class="clearfix module-wrapper product-info" style="padding-right:16px;">
            <p class="model_title"><i class="el-icon-tickets"></i> 产品信息</p>
            <div class="gy-form-group">
                <span class="l">企业类型</span>
                <span>{{enterpriseParent?enterpriseParent + ', ' + enterpriseChild:'暂无数据'}}</span>
            </div>
            <table v-if="viewData.enterpriseParentId==10" class="gy-table product_list">
                <thead>
                <tr>
                    <td>品类</td>
                    <td>产品</td>
                ...
                <tr v-for="(item,index) in viewData.companyProductInfoModelList" :key="index+50">
                    ...
                    <td class="align-r">{{item.annualConsumption|dataFormat}}</td>
                    <td class="align-r">{{item.productionOutput|dataFormat}}</td>
                    <td>{{item.remark|dataFormat}}</td>
                </tr>
                </tbody>
            </table>

            <table v-if="viewData.enterpriseParentId==12" class="gy-table product_list">
                <thead>
                <tr>
                    <td>主营品类</td>
                    <td>主营产品</td>
        ...
        <!-- 证件信息 -->
        <div class="clearfix module-wrapper document-info">
            <p class="model_title company-information-view"><i class="el-icon-tickets"></i> 证件信息</p>
            <div class="img_show_div" v-for="(item,index) in  viewData.companyFileModelList"
                 v-if="item.rskDeclareCompanyFileTypeId>4" :key="index+1000">
                <div class="img_file_con">
                    <span class="img_file_name">{{item.fileName|dataFormat}}</span>
                    <div class="file_img_list">
                        <div v-for="(n, i) in item.filePathList" :key="i + 900" @click="showImg(n)"><img :src="n"
                                                                                                         alt=""></div>
                    </div>
                </div>
                <div class="file_info  clearfix">
                    <div class="gy-form-group" v-for="(subItem, subIndex) in item.fileParamValueModelList"
                         :key="subIndex+100">
            ...
            <div style="margin-left:15px" v-if="viewData.companyFileModelList.length <= 0">暂无数据</div>
        </div>
        <!-- 经营资质 -->
        <div class="clearfix module-wrapper operating-qualifications">
            <p class="model_title"><i class="el-icon-tickets"></i> 经营资质</p>
            <div class="gy-form-group  single-row" style="padding-right:16px;">
                <span class="l">装备设施</span>
                <span>{{viewData.devices|dataFormat}}</span>
            </div>
        </div>
        <div v-if="viewData.remark" class="remark_con module-wrapper">
    ...
    methods: {
        ...
        getViewData () {
            ...
            if (this.historyCompanyInfo) {
                httpApi = this.$api.ratingApplication.getMgmtRatingFormDataMgmt + this.historyCompanyInfo;
            } else {
