<template><!-- 买家结算 -->
    <div class="order">
        <el-card class="box-card">
            ...
            <div class="state-pannel" style="overflow: inherit;">
                <template>
                    <div style="margin-left: -5px;">
                        <i style="font-size: 12px;
    color: #333;
    margin-right: 2px;
    vertical-align: bottom;" class="iconfont icon-zijinzhanghuicon"></i>
                        <span style="font-size: 14px;
    font-weight: bold;
    color: #333;">结算与复核状态</span>
                    </div>
                ...
                <el-dialog width="1062px" class="settle-dialog" title="订单结算申请"
                    ...
                    <div class="new-title"><i class="iconfont icon-settle"></i> <span>结算明细</span></div>
                    <table class="gy-table">
                        ...
                        <tr v-if="tableDataList">
                            <td colspan="4" style="text-align: left;font-weight: bold">汇总</td>
                    ...
                    <div class="new-title2"><i class="iconfont icon-settle"></i> <span>付款明细</span></div>
                    <table class="gy-table">
                        <thead>
                        <tr>
                            <td>No</td>
                        ...
                        <tr v-if="paymentListDate">
                            <td colspan="5" style="text-align: left;font-weight: bold">汇总</td>
                            <td>{{sumMpayTotal}}</td>
                        </tr>
                        </tbody>
                    </table>
                    <div class="new-title2"><i class="iconfont icon-settle"></i> <span>其他费用</span></div>
                    ...
                    <div class="settleInfo-totol">
                        ...
                        <span class="newspan">{{orderMoney |numToCash}} 元</span>
                    </div>
                    <div class="settleInfo-totol">
                        <span class="settleInfo-money">已付款总金额:</span>
                        <span class="newspan">{{paidMoney.toFixed(2) |numToCash}} 元</span>
                    </div>
                    <div class="settleInfo-totol">
                        <span class="settleInfo-money">申请结算总金额:</span>
                        <span class="newspan">{{settleAmount + orderSettles |numToCash}} 元</span>
                    </div>
                    <div class="settleInfo-totol">
                        <p>
                            <span class="settleInfo-money settleInfo-pay">需支付余款:</span>
                            <span class="settleInfo-money-color newspan">{{settleAmount-paidMoney + orderSettles |numToCash}} 元</span>
                            <span class="settleInfo-money settleInfo-pay">对方发货总量:</span>
                            <span class="settleInfo-money-color newspan">{{quantityLoading}} 吨</span>
                        </p>
                    </div>
                    <span slot="footer" class="dialog-footer">
                        <button class="gy-button-extra" @click="dialogVisible = false">关闭</button>
                    </span>
                </el-dialog>
            </div>
        </el-card>
        <div>
            ...
            <el-row>
                <el-col :span="24">
                    <gy-order-settle @showContract="showContract" :order-data="info" :order-view="previewerImgOrder" :order-img="prebiewImg" v-if="info" user-type="buy"></gy-order-settle>
                </el-col>
            </el-row>
...
export default {
    ...
    methods: {
        ...
        postProcesses (aa) {
            ...
            that.settleConfirm.operaType = aa;
        ...
        postStartSettlementOrderMsg () {
            ...
            that.$http.post(that.$api.order.startSettlementOrderMsg, that.startSettlementOrderMsg)
                .then(function (res) {
                    ...
                    that.payClick();
                    if (res.data.data.approveStatus === 0) {
                        // 卖家审批中
                        that.resour = false;
                    } else if (res.data.data.approveStatus === 3) {
                        // 买家审批中
                        that.resour = false;
                        // 再次判断是否要强控(这里由于异步调用的原因，不一定能取到从getOrderInfo中的数据)
                        that.doCheckApproveStatus(res.data.data.settleId, function (data) {
                            if (data.rsltStatus === 1 && data.controlStatus === 1) {
                                // 审批中，且强控
        ...
        checkApproveStatus (settleId) {
            let that = this;
            that.doCheckApproveStatus(settleId, function (data) {
                if (data.rsltStatus === 1) {
                    that.isApproving = true;
                }
            });
        },
        doCheckApproveStatus (settleId, func) {
            ...
            that.$http.get(that.$api.invoice.sellerInvoiceImg + '/' + item.id)
                .then((res) => {
                    ...
                    if (that.prebiewImg.list.length > 0) {
                        ...
                        console.log(that.prebiewImg, '222222222');
                    } else {
