    ...
    <div class="transport-wrap order">
        ...
        <div class="t">
            ...
            <div class="search">
                ...
                <input type="text" placeholder="请输入品名、单号" v-model="orderData.data.keywords" class="gy-input" @keyup.13="handleSearch">
                <i class="iconfont" :class="{'icon-arrow-down': !showAdvance, 'icon-arrow-up': showAdvance}" @click="showAdvance = !showAdvance"></i>
            </div>
            <transport-search :show-advance="showAdvance" :keywords="orderData.data.keywords" :advance.sync="orderData.data" :search-info.sync="orderInfo" :search-type="searchType" :count-status.sync="statusList"></transport-search>
        </div>
