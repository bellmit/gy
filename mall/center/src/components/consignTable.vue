    ...
    <table class="gy-table list">
        ...
        <tr v-for="(item, index) in orderList" :key="index">
            <td :colspan="theadList.length" class="item">
                <div class="item-t">
                    <span class="fl" v-if="No === 0 || No === 3">单号：{{item.consignmentNoteCode}}
                        <!--<i class="iconfont icon-copy"></i>--></span>
                    <span class="fl" v-if="No === 2">单号：{{item.enquiryNoteCode}}
                        <!--<i class="iconfont icon-copy"></i>--></span>
                    <span class="fr">创建时间：{{item.createdDate | date(true)}}</span>
                </div>
                ...
                <table class="gy-table" v-if="No === 2">
                    <tr>
                        ...
                        <td class="td-108">{{item.estimatedLoadingDate | date}}</td>
                        <td class="td-222">装货地：<br>{{item.loadTotalAddress}}<br>卸货地：<br>{{item.unloadTotalAddress}}</td>
...
export default {
    data () {
        return {
            orderStatusValue: ['签约中', '签约中', '已失效', '实施中', '结算中', '结算中', '结算中', '付款中', '已完成', '实施中', '实施中', '实施中'],
