        ...
        <tr v-for="(item ,index ) in payList" :key="index">
            ...
            <td>
                <template v-if="item.payBillType === 1 && userType !== 'sell'">
                    <router-link :to="{name: 'paymentInfo', query:{paymentId:item.id}}" class="yel">查看</router-link>
                </template>
                <template v-else-if="item.payBillType === 2 && userType !== 'sell'">
                    <router-link :to="{name: 'collectionInfo', query:{collectionId:item.colId}}" class="yel">查看
                    </router-link>
                </template>
                <template v-else-if="item.payBillType === 1 && userType === 'sell'">
                    <router-link :to="{name: 'collectionInfo', query:{collectionId:item.id}}" class="yel">查看
                    </router-link>
                </template>
                <template v-else-if="item.payBillType === 2 && userType === 'sell'">
                    <router-link :to="{name: 'paymentInfo', query:{paymentId:item.paymentId}}" class="yel">查看
                    </router-link>
                </template>
            </td>
        </tr>
