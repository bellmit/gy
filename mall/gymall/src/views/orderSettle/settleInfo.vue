            ...
            <div class="state-pannel">
                <template>
                    ...
                    <div class="button-group">
                        ...
                        <button class="gy-button-extra" @click="openVisible()">发票</button>
                    </div>
                    <el-dialog width="1062px" class="settle-dialog"
                        ...
                        <table class="gy-table">
                            ...
                            <tr v-for="(item, index) in tableData" :key="index">
                                ...
                                <td><input type="number" placeholder="请输入结算数量" v-model="item.settlementQuantity"></td>
                                <td><input type="text" placeholder="请输入结算单价" v-model="item.settlementPrice"></td>
                        ...
                        <div class="settleInfo-totol">
                            <p>
                                ...
                                <span class="settleInfo-money-color">{{settleAmount-paidMoney.toFixed(2) |numToCash(3)}}</span>
                            </p>
                        </div>
                        <div class="settleInfo-totol">
                            <button class="gy-button-extra" @click="handleToSettle()" v-if="resour">确认发起结算</button>
                            <button class="gy-button-extra" @click="dialogVisible = false" v-if="!resour">关闭</button>
                        </div>
                    </el-dialog>
                    <el-dialog width="600px" height="600px" class="settle-dialogs"
            ...
            <el-row>
                <el-col :span="24">
                    <div class="base-info">
                        ...
                        <dl>
                            <dt>交付方式</dt>
                            <dd>{{info.deliveryType === 0 ? '买家自提、卖家代运都可以' : info.deliveryType === 1 ? '买家自提' :
                                '卖家代运'}}
                            </dd>
                        </dl>
                        <dl>
                    ...
                    <el-tab-pane label="付款明细" name="second">
                        <table>
                            ...
                            <tr>
                                ...
                                <td>已支付</td>
                                <td>
                                    <router-link :to={} class="yel">查看</router-link>
                                </td>
                            </tr>
                            <tr>
                                <td>180021545</td>
                    ...
                    <el-tab-pane label="货物交割" name="three">
                        <table class="transaction">
                            ...
                            <tr class="ge" v-for="(order, index) in item.deliveryOrderItemList" :key="index">
                                ...
                                <td>发货仓库：{{item.loadingWarehouseName}}<br>发货时间：{{item.estimatedArrivalStart |
                                    date(1)}}<br>签收时间：{{item.estimatedArrivalEnd | date(1)}}
                                </td>
                                <td>
                                    <router-link :to="{}">
                    ...
                    <el-tab-pane label="物流明细" name="four">
                        <table>
                            ...
                            <tr v-for="(tran,index) in item.consignmentNoteItemList" :key="index">
                                ...
                                <!--id-->
                                <td>
                                    <router-link :to={} class="gy-button-view">查看调度</router-link>
                                </td>
                            </tr>
                            </tbody>
                        </table>
    ...
    methods: {
        ...
        sureUrls () {
            this.cashData.img.map(item => {
                ...
                item.invoiceUrl = this.settleInfoUrl;
            });
        ...
        transaction () {
            let that = this;
            that.$http.get(that.$api.order.transaction + that.orderId)
                .then(function (res) {
        ...
        logisticss () {
            ...
            that.$http.post(that.$api.transport.orderList, that.params)
                .then(function (res) {
        ...
        getOrderInfo () { // 获取订单信息
            let that = this;
            console.log(that.$api.order.sellerSettle, {'orderId': that.orderId, 'userId': that.userId});
        ...
        settleInfoImg (file) {
            ...
            that.$http.post(that.$api.invoice.settleInvoice, formData, headers)
                .then(function (res) {
                    that.settleInfoUrl = res.data.data;
