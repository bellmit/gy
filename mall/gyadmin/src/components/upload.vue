...
export default {
    ...
    methods: {
        ...
        beforeUploadHandler (file) {
            if ((this.listType === 'picture' || this.listType === 'picture-card') && file.type !== 'image/png' && file.type !== 'image/jpeg' && file.type !== 'image/jpg' && file.type !== 'application/pdf' && file.type !== 'image/bmp') {
                this.$message({
                    showClose: true,
                    message: `文件扩展名错误`,
                    type: 'error'
        ...
        onUpload (file) {
            ...
            this.$http.post(this.url || this.$api.transport.uploadImage, formData, headers)
                .then(res => {
