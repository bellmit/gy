...
<template>
    <gy-item :title="gyItem.title" :titleType="gyItem.titleType" :active="gyItem.active"
             :isBig="gyItem.isBig" :resetData="gyItem.resetData" id="gyTradingMoney">
        <div class="item-content-item" v-show="Statistic && Statistic.length > 0" id="myChart4" style="width: 480px;height: 208px;"></div>
...
export default {
    ...
    methods: {
        // 交易品类(交易额)
        statistic3 (t) {
            this.$http.get(this.$api.charts.statistics + '?statisticsId=15&countType=' + t)
                .then((res) => {
                    if (res.data.code === 0) {
        ...
        drawPie1 () {
            ...
            for (let idx of this.Statistic.keys()) {
                ...
                item.total = ((this.Statistic[idx].total) / 10000).toFixed(2);
                item.itemStyle = {color: this.$constant.chartsColor[idx]};
                lenged.push(item);
            }
            // document.getElementById('myChart4').style.width = document.getElementById('gyTradingMoney').offsetWidth - 2 + 'px';
            let myChart = this.$echarts.init(document.getElementById('myChart4'));
            myChart.setOption({
                tooltip: {
                    formatter: (item) => {
                        return item.marker + item.data.name + ' ' + this.$tools.numToCash(item.data.total) + ' 万元';
                    }
                },
                legend: {
                    orient: 'vertical',
