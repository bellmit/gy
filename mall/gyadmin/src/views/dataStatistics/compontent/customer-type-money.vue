...
<template>
  <gy-item :title="gyItem.title" :titleType="gyItem.titleType" :active="gyItem.active"
           :isBig="gyItem.isBig" :resetData="gyItem.resetData" id="gyCustomerMoney">
    <div class="item-content-item" v-show="Statistic && Statistic.length > 0" id="myChart5" style="width: 480px;height: 208px;"></div>
...
export default {
    ...
    methods: {
        // 客户类型(交易额)
        statistic5 (t) {
            this.$http.get(this.$api.charts.statistics + '?statisticsId=19&countType=' + t)
                .then((res) => {
                    if (res.data.code === 0) {
        ...
        drawPie1 () {
            ...
            // document.getElementById('myChart5').style.width = document.getElementById('gyCustomerMoney').offsetWidth - 2 + 'px';
