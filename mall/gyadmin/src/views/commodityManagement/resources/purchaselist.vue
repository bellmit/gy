<template>
  <div class="commodity-main list-resources">
        ...
        <div class="right_div">
          <div class="right_div1">
            <div class="search" style="position:relative">
              <input type="search" v-model="keywords" placeholder="请输入需求单号/品名/公司名" class="gy-input">
    ...
    <div>
      <div class="right">
        <button type="button" class="gy-button-extra" @click="goNewPage('purchaseAdd')">新增求购</button>
      ...
      <table class="gy-table" style="width:99.9%;">
        <thead>
        <tr class="top">
          <th style="width:30px">
        ...
        <tr class="item">
          ...
          <td>
            ...
            <template v-if="sub.offerStatus === 0">
              <span class="gy-button-view" @click="updateState(1,sub.id)">下架</span><br>
            </template>
            <template v-if="sub.offerStatus === 1">
              <span class="gy-button-view" @click="updateState(0,sub.id)">上架</span><br>
              <!--<router-link :to="{name:'createResources', query:{offerId: sub.id}}" class="gy-button-view"> </router-link><br>-->
            </template>
            <span @click="goNewPage('/resources/purchasedetail', sub.id)"
                  class="gy-button-view">详情</span><br>
            <!-- <span @click="$router.push({name:'createOrder', query:{offerId: sub.id}})" v-if="sub.offerStatus === 0"
...
<script>export default {
    ...
    methods: {
        ...
        templateDownload () {
            this.$http.get(this.$api.offers.templateDownload + 2, {responseType: 'blob', timeout: 60000}).then(res => {
                if (res.data) {
                    this.download(res.data, '求购列表资源单模版.xls');
                    return;
                }
        ...
        upload (file) {
            ...
            this.$http.post(this.$api.offers.buyer, formData, headers)
                .then(function (res) {
        ...
        batchOperation (state) {
            ...
            if (this.datas.offerIdList.length) {
                ...
                if (this.datas.offerStatus === 0) {
                    console.log(this.datas.offerStatus);
                    this.updateStatesBatch(this.datas, this.$api.offers.buyBatchUp);
                }
                // 批量下架
                if (this.datas.offerStatus === 1) {
                    console.log(this.datas.offerStatus);
                    this.updateStatesBatch(this.datas, this.$api.offers.buyBatchDown);
                }
        ...
        search (state) { // 搜索
            ...
            if (state === 1) {
                this.$tools.downloadFile(this, 'platform/v1/by/exportExcel', this.parameter.data, '求购单.xls');
            }
        },
        // downloadFile (par) {
        //     this.$http({
        //         method: 'post',
        //         url: 'platform/v1/offers/exportExcel',
        //         responseType: 'blob',
        //         data: par
        //     }).then(res => {
        ...
        updateState (status, parm) {
            ...
            // 上架
            if (this.datas.offerStatus === 0) {
                this.updateStatesd(this.datas, this.$api.offers.buyUp);
            }
            // 下架
            if (this.datas.offerStatus === 1) {
                this.updateStatesd(this.datas, this.$api.offers.buyDown);
            }
            if (this.datas.offerStatus === 2) {
                this.updateStates(this.datas);
            }
