<template>
  <div class="commodity-main list-resources">
    ...
    <div class="selected-box" v-show="toggleSelect">
      <el-form label-width="80px" size="mini">
        ...
        <div class="gy-form-group">
          ...
          <i class="iconfont icon-search" style="right: 50px" @click="onelist1click"></i>
    ...
    <div>
      <div class="right">
        <button type="button" class="gy-button-extra" @click="$router.push({path: 'add'})">新增资源</button>
      ...
      <table class="gy-table">
        ...
        <tr class="update-date">
          <td colspan="2">单号：{{sub.odrOfferSn}}</td>
          <td colspan="7" style="text-align: right;padding-right:10px">发布日期：{{sub.createdDate|date}}</td>
        </tr>
        <tr class="item">
          <td><label>
          ...
          <td>{{sub.skuQuantity|numToCash(3)}}{{sub.infUnitOfMeasureDisplayName}}</td>
          <td>
          ...
          <td>
            ...
            <span @click="$router.push({name:'resourcesView', query:{offerId: sub.id}})"
...
<script>export default {
    ...
    methods: {
        ...
        search (state) { // 搜索
            ...
            if (state === 1) {
                this.$tools.downloadFile(this, 'platform/v1/offers/exportExcel', this.parameter.data);
            }
        },
        // downloadFile (par) {
        //     this.$http({
