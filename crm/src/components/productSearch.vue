<template>
    <div class="product-list">
        ...
        <ul v-show="showList" v-clickOutside="handleHiddenList">
            <li v-for="(item, index) in list" :key="index" @click="handleList(item)" v-if="list.length > 0">{{item.name}}</li>
...
export default {
    ...
    methods: {
        ...
        handleGetList () {
            this.$http.get(this.$api.client.matchmaking + '?companyName=' + encodeURI(this.keywords)).then(res => {
